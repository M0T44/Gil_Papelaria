...
export default function AuthContext({ children }) {
    ...
    async function handleClearToken() {
        const iPedido = await AsyncStorage.getItem('id_pedido')
        const id_pedido = JSON.parse(iPedido)
    ...
    return (
        <Context.Provider value={{ autenticado, handleLogar, handleClearAsync, handleRealizarPedido, handleClearToken }}>
            {children}
        </Context.Provider>
    )
}